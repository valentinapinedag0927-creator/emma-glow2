    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-white pointer-events-none overflow-hidden animate-[fadeOut_1s_ease-in-out_forwards]">
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          <div className="absolute z-20 animate-[ping_0.8s_cubic-bezier(0,0,0.2,1)_infinite]">
              <Sparkles size={180} className="text-yellow-500 fill-yellow-400 drop-shadow-[0_0_50px_rgba(255,215,0,1)]" />
          </div>
          {randomSparkles.map((s) => (
              <div key={s.id} className="absolute bg-yellow-300 rounded-full shadow-[0_0_10px_#FDB022] animate-twinkle"
...
const App: React.FC = () => {
  ...
  const handleLogin = (username: string) => { setUser({ ...user, username, isLoggedIn: true }); setView(ViewState.HOME); };
  const handleLogout = () => { setUser({ username: '', isLoggedIn: false, history: [], routineHistory: [], trackers: [] }); setView(ViewState.LOGIN); };
  const handleAnalysisComplete = (result: AnalysisResult) => { setUser(prev => ({ ...prev, history: [result, ...prev.history] })); };
  ...
  return (
    <div className="min-h-screen text-black pb-20 md:pb-0 font-sans relative overflow-x-hidden bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
      ...
      <div className="fixed top-20 right-20 w-56 h-56 bg-yellow-200 rounded-full blur-3xl opacity-40 animate-blob animation-delay-2000 pointer-events-none -z-10"></div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 md:py-24">
        ...
        {view === ViewState.ANALYSIS && <Analysis user={user} onAnalysisComplete={handleAnalysisComplete} />}
        {view === ViewState.INGREDIENTS && <IngredientScanner user={user} />}
        {view === ViewState.ROUTINE_BUILDER && <RoutineBuilder />}
        {view === ViewState.SMART_MIRROR && <SmartMirror />}
        {view === ViewState.ADVISOR && <Advisor />}
        {view === ViewState.SHOP && <Shop addToCart={addToCart} cart={cart} removeFromCart={removeFromCart} />}
        {view === ViewState.EDUCATION && <Education />}
        {view === ViewState.PRODUCT_HISTORY && <ProductHistory user={user} onUpdateUser={setUser} />}

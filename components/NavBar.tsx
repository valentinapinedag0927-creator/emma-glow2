...
const NavBar: React.FC<NavBarProps> = ({ currentView, setView, cartCount, onLogout }) => {
  const navItems = [
    ...
    { id: ViewState.ANALISIS, icon: Camera, label: 'Análisis' },
    ...
    <nav className="fixed bottom-0 md:bottom-auto md:top-0 w-full z-50 glass-panel md:rounded-b-[2rem] border-t md:border-t-0 md:border-b-2 border-white/60 shadow-lg shadow-pink-100/40">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16 md:h-24">
          ...
          {/* Logo - He añadido whitespace-nowrap para que no se vea vertical */}
          <div className="hidden md:flex items-center space-x-3 cursor-pointer group" onClick={() => setView(ViewState.HOME)}>
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-pink-300 to-[#ec4899] flex items-center justify-center text-white font-serif font-bold text-3xl shadow-md group-hover:scale-110 transition-transform border-2 border-white ring-2 ring-pink-100">
              E
            </div>
            <div className="text-[#ec4899] mt-2 transform group-hover:translate-x-1 transition-transform whitespace-nowrap">
              <AbstractText text="Emma Glow" baseSize="md" align="left" className="tracking-wide font-bold" />
            </div>
          </div>

          {/* Nav Links - Con el color rosado oscuro corregido */}
          <div className="flex flex-1 justify-around md:justify-end md:space-x-2 overflow-x-auto no-scrollbar py-2">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => setView(item.id)}
                className={`flex flex-col md:flex-row items-center space-y-1 md:space-y-0 md:space-x-2 p-2.5 rounded-2xl transition-all whitespace-nowrap ${
                  currentView === item.id
